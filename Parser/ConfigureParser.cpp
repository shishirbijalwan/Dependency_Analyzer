...
ConfigParseToConsole::~ConfigParseToConsole()
{
	...
	delete pAliasCheck;
	delete pTypeDefCheck;
	delete pAddTypeDefToTypeTable;
	delete pAddAliasToTypeTable;
	pIn->close();
	delete pIn;
}
...
Parser* ConfigParseToConsole::Build()
{
	...
	{	pToker = new Toker;
		...
		pAddToTypeTableFunction = new AddToTypeTableFunction(pRepo);
		pFunctionDefinition->addAction(pAddToTypeTableFunction);
		...
		pAddTypeClassAndNameSpace = new AddTypeClassAndNameSpace(pRepo);
		pClassAndNameSpace->addAction(pAddTypeClassAndNameSpace);
		pClassAndNameSpace->addAction(pPushClassAndNameSpace);
		pParser->addRule(pClassAndNameSpace);		
		//Configure to detect Alias
		pAliasCheck = new AliasCheck();
		pAddAliasToTypeTable = new AddAliasToTypeTable(pRepo);
		pAliasCheck->addAction(pAddAliasToTypeTable);
		pParser->addRule(pAliasCheck);
		//Configure to detect TypeDef
		pTypeDefCheck = new TypeDefCheck();
		pAddTypeDefToTypeTable = new AddTypeDefToTypeTable(pRepo);
		pTypeDefCheck->addAction(pAddTypeDefToTypeTable);
		pParser->addRule(pTypeDefCheck);
		return pParser;	}
